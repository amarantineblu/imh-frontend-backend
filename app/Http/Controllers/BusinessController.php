...
class BusinessController extends Controller
{
    ...
    */
    ...
    public function getRegister()
    {
        ...
        return Inertia::render('sign', compact(
            'currencies',
            'timezone_list',
            'months',
            'accounting_methods',
    ...
    public function postRegister(Request $request)
    {
        ...
        try {
            $validator = $request->validate(
                ...
                [
                    'business_name.required' => __('validation.required', ['attribute' => __('business.business_name')]),
            ...
            //Create owner.
            $owner_details = $request->only(['prefix', 'first_name', 'last_name', 'username', 'email', 'password', 'language']);

            $owner_details['language'] = empty($owner_details['language']) ? config('app.locale') : $owner_details['language'];

            $user = User::create_user($owner_details);

            $business_details = $request->only(['business_name', 'start_date', 'currency_id', 'time_zone',
                'fy_start_month', 'accounting_method', 'tax_label_1', 'tax_number_1',
                'tax_label_2', 'tax_number_2', ]);

            $business_location = $request->only(['business_name', 'country', 'state', 'city', 'zip_code', 'landmark',
                'website', 'mobile', 'alternate_number', ]);

            //Create the business
            $business_details['owner_id'] = $user->id;
            if (! empty($business_details['start_date'])) {
                $business_details['start_date'] = Carbon::parse($business_details['start_date'])->format('m/d/Y');
